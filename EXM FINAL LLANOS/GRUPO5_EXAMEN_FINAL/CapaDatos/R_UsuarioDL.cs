            ...
            using (SqlConnection cn = new ConexionBD().conectar())
            {
                try
                {
                    ...
                    using (SqlDataReader reader = cmd.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            lista.Add(new R_Usuario(int.Parse(reader[0].ToString()), reader[1].ToString(),reader[2].ToString(), reader[3].ToString(), reader[4].ToString(), reader[5].ToString(), reader[6].ToString(), reader[7].ToString()));
                        }
                    }
                    cn.Close();
                }
            ...
            using (SqlConnection cn = new ConexionBD().conectar())
            {
                ...
                try
                {
                    ...
                    using (SqlDataReader dr = cmd.ExecuteReader())
                    {
                        if (dr.Read())
                        {
                            or_Usuario = new R_Usuario(int.Parse(dr[0].ToString()), dr[1].ToString(), dr[2].ToString(), dr[3].ToString(), dr[4].ToString(), dr[5].ToString(), dr[6].ToString(), dr[7].ToString());
                        }
                        else
                        {
                            or_Usuario = null; 
